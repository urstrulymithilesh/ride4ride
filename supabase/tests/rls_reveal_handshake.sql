...
select case
         when count(*) = 1 and bool_and(owner_agreed = false)
                          and bool_and(agreed = false)
           then 'PASS' else 'FAIL' end
         as test_1_viewer_cannot_self_grant_owner_agreed,
       bool_and(owner_agreed) as owner_agreed,
       bool_and(agreed)       as agreed
...
select case when count(*) = 0 then 'PASS' else 'FAIL' end
         as test_2_viewer_still_blocked_after_self_grant,
...
set local request.jwt.claims = '{"sub":"00000000-0000-0000-0000-00000000000b","role":"authenticated"}';
select case
         when count(*) = 1 and bool_and(from_address is not null)
                          and bool_and(to_address   is not null)
           then 'PASS' else 'FAIL' end
         as test_3_addresses_readable_after_mutual_agreement,
       count(*) as rows_visible
  from public.ride_locations
 where ride_id = '11111111-1111-1111-1111-111111111111';
reset role;
